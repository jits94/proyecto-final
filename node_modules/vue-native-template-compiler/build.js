...
function filterDirective(ast) {
  ...
  if (Array.isArray(ast.directives)) {
    return ast.directives.filter(function(v) {
...
function filterDirectiveBindProps(ast) {
  if (Array.isArray(ast.directives)) {
    return ast.directives.filter(function(v) {
      return v.name === 'bind' && v.value === '$props'
    })
  } else {
    return []
  }
...
BaseGenerator.prototype.setVariableDependency = function setVariableDependency(
  variable,
) {
...
BaseGenerator.prototype.genDependence = function genDependence() {
  var code = ''
  var helperDependency = this.variableDependency.filter(function(v) {
    return v !== COMMON.createElement && v !== COMMON.component && v.alias
  })
  if (helperDependency.length) {
    code += 'import { '
    code += helperDependency
      .map(function(v) {
        return v.alias + ' as ' + v.name
      })
      .join(',')
  ...
  code +=
    'import {\n      ' +
    COMMON.createElement.alias +
    ' as ' +
    COMMON.createElement.name +
    ',\n      ' +
    COMMON.component.alias +
    ' as ' +
...
function wrapFilter$1(exp, filter) {
  ...
  if (i < 0) {
    // _f: resolveFilter
    return (
      COMMON.resolveFilter.name + '.call(vm, "' + filter + '")(' + exp + ')'
    )
  } else {
    var name = filter.slice(0, i)
    var args = filter.slice(i + 1)
    return (
      COMMON.resolveFilter.name +
      '.call(vm, "' +
      name +
      '")(' +
      exp +
...
var RenderGenerator = /*@__PURE__*/ (function(BaseGenerator) {
  ...
  RenderGenerator.prototype = Object.create(
    BaseGenerator && BaseGenerator.prototype,
  )
   ...
   * @param {Object} ast
   */
  RenderGenerator.prototype.genTextExpression = function genTextExpression(
    ast,
  ) {
  ...
  RenderGenerator.prototype.genIfConditions = function genIfConditions(
    conditions,
  ) {
  ...
  RenderGenerator.prototype.genFor = function genFor(ast) {
    ...
    var code =
      ...
      iterator2 +
      '){return ' +
      this.genElement(this.genKeyFor(ast)) +
      '}.bind(this))'
    return code
  }
  ...
  RenderGenerator.prototype.genSlot = function genSlot(ast) {
    ...
    var code =
      COMMON.renderSlot.value + '(' + name + ', {' + props.join(',') + '})'
   ...
   * @param {Object} ast
   */
  RenderGenerator.prototype.genTransitionGroup = function genTransitionGroup(
    ast,
  ) {
    return this.genChildren(ast)
  }
    ...
    ast.directives.forEach(function(v) {
      code.push(
        '{name:"' +
          v.name +
          '",directiveName:"' +
          changeCase.lowerCase(changeCase.camelCase(v.name)) +
          '",rawName:"' +
          v.rawName +
          '"' +
          (v.value
  ...
  // eslint-disable-next-line no-unused-vars
  RenderGenerator.prototype.genDirectiveContext = function genDirectiveContext(
    ast,
  ) {
    var code = ''
  ...
  RenderGenerator.prototype.genRef = function genRef(ast) {
    ...
    if (ast.parent === undefined) {
      ...
      code2 =
        "this.setRootRef(ref);this.props['" +
        COMMON.setRef.name +
        "'] && this.props['" +
        COMMON.setRef.name +
        "'](ref);"
  ...
  RenderGenerator.prototype.isWebInput = function isWebInput(ast) {
    if (!ast.inputProcessed) {
      if (ast.tag === 'textarea') {
        ...
        var type = ast.attrs.filter(function(v) {
          return v.name === 'type'
        })[0]
        if (type === undefined) {
          return true
...
function genHandlers(events, options) {
  ...
  for (var name in events) {
    ...
    if (name.indexOf('~') === 0) {
      eventHandler =
        'this.setEventOnce(function once_' +
        ++uid +
        '(event){(' +
        eventHandler +
        ')(event)})'
    ...
    res +=
      'on' +
      changeCase.pascalCase(addSeparateLine(name)) +
      ': ' +
      eventHandler +
      ','
  }
...
function genTransitionEventHandler(name, handler) {
  ...
  if (Array.isArray(handler)) {
    return (
      ...
      handler
        .map(function(handler) {
          return genTransitionEventHandler(name, handler)
        })
        .join(',') +
      ']'
    )
...
var ReactNativeRenderGenerator = /*@__PURE__*/ (function(RenderGenerator) {
  ...
  ReactNativeRenderGenerator.prototype = Object.create(
    RenderGenerator && RenderGenerator.prototype,
  )
   ...
   * @param {Object} ast
   */
  ReactNativeRenderGenerator.prototype.genTextExpression = function genTextExpression(
    ast,
  ) {
  ...
  // merge style & class
  ReactNativeRenderGenerator.prototype.genNativeStyleProps = function genNativeStyleProps(
    ast,
  ) {
    ...
    return (
      'style: ' +
      NATIVE.mergeStyleAndClass.name +
      '(' +
      classProps +
      ', ' +
      styleProps +
   ...
   * @param {Object} ast
   */
  ReactNativeRenderGenerator.prototype.genStyleProps = function genStyleProps(
    ast,
  ) {
    var styleAttrsValue = ast.attrs
      .filter(function(v) {
        return v.name === 'style'
      })
      .map(function(v) {
    ...
    var show =
      ast.directives &&
      ast.directives.filter(function(v) {
        return v.name === 'show'
      })[0]
   ...
   * @param {Object} ast
   */
  ReactNativeRenderGenerator.prototype.genClassProps = function genClassProps(
    ast,
  ) {
    var topParent = this.isAstTopParent(ast)
    var classAttrsValue = ast.attrs
      .filter(function(v) {
        return v.name === 'class'
      })
      .map(function(v) {
  ...
  ReactNativeRenderGenerator.prototype.isAstTopParent = function isAstTopParent(
    ast,
  ) {
    if (ast.parent === undefined) {
      return true
...
function genRadioModel(el, value, modifiers) {
  ...
  addAttr(
    el,
    'checked',
    COMMON.looseEqual.name + '(' + value + ',' + valueBinding + ')',
  )
...
function genSelect(el, value, modifiers) {
  ...
  var selectedVal =
    ...
    'return ' +
    (number ? COMMON.toNumber.name + '(val)' : 'val') +
    '})'
...
function html(ast) {
  var obj = {}
  var directive = ast.directives.filter(function(v) {
    return v.name === 'html'
  })[0]
...
var ReactWebRenderGenerator = /*@__PURE__*/ (function(RenderGenerator) {
  ...
  ReactWebRenderGenerator.prototype = Object.create(
    RenderGenerator && RenderGenerator.prototype,
  )
   ...
   * override
   */
  ReactWebRenderGenerator.prototype.genTransition = function genTransition(
    ast,
  ) {
    var this$1 = this
    ...
    var arr = []
    var i = 0
    ast.children.forEach(function(v1) {
      if (v1.if) {
        var conditionsArr = []
        v1.ifProcessed = true
        v1.ifConditions.forEach(function(v2) {
          conditionsArr.push(
            '{\n            index: ' +
              i++ +
              ',\n            exp: ' +
              (v2.block.else ? true : v2.exp) +
              ',\n            element: ' +
              this$1.genElement(v2.block) +
              '\n          }',
          )
        })
        arr.push(
        ...
        v1.directives.filter(function(v) {
          return v.name === 'show'
        }).length
      ) {
   ...
   * override unfinished
   */
  ReactWebRenderGenerator.prototype.genTransitionGroup = function genTransitionGroup(
    ast,
  ) {
    var node = {
      tag: 'span',
  ...
  ReactWebRenderGenerator.prototype._genTransitionGroup = function _genTransitionGroup(
    ast,
  ) {
    var this$1 = this
   ...
   * @param {Object} ast
   */
  ReactWebRenderGenerator.prototype.genClassProps = function genClassProps(
    ast,
  ) {
    var code = ''
    var topParent = this.isAstTopParent(ast)
    ...
    code =
      'className: (' + code.trim().replace(/^[\s]*\+[\s]*/, '') + ').trim()'
    return code
  }
   ...
   * @param {Object} ast
   */
  ReactWebRenderGenerator.prototype.genStyleProps = function genStyleProps(
    ast,
  ) {
    var styleAttrsValue = ast.attrs
      .filter(function(v) {
   ...
   * @param {Object} ast
   */
  ReactWebRenderGenerator.prototype.genIMEResolve = function genIMEResolve(
    ast,
  ) {
    ast.attrs = ast.attrs || []
    ast.attrs.push({
  ...
  ReactWebRenderGenerator.prototype.genDirectives = function genDirectives(
    ast,
  ) {
    var this$1 = this
  ...
  ReactWebRenderGenerator.prototype.genModelDirectives = function genModelDirectives$1(
    ast,
    directive,
  ) {
  ...
  ReactWebRenderGenerator.prototype.genHtmlDirectives = function genHtmlDirectives$1(
    ast,
  ) {
    return html(ast)
  }

  ReactWebRenderGenerator.prototype.genTextDirectives = function genTextDirectives$1(
    ast,
  ) {
    return text(ast)
  }

  ReactWebRenderGenerator.prototype.genEventHandler = function genEventHandler(
    ast,
  ) {
    var code = ''
    if (ast.events) {
  ...
  ReactWebRenderGenerator.prototype.genNativeEventHandler = function genNativeEventHandler(
    ast,
  ) {
    var code = ''
    if (ast.nativeEvents && !isReservedTag(ast.tag)) {
  ...
  ReactWebRenderGenerator.prototype.isAstTopParent = function isAstTopParent(
    ast,
  ) {
    if (ast.parent === undefined) {
      return true
...
function traverse(ast, options, importObj, parent, childIndex) {
  ...
  if (
    ...
    var slotname = ast.attrsMap['render-prop'] || ast.attrsMap['render-prop-fn']
  ...
  if (ast.children) {
    ast.children.forEach(function(child, index) {
      if (child.ifConditions) {
        child.ifConditions.forEach(function(condition, conditionIdx) {
