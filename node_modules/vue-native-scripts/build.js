...
function compileVueToRn(resource, filename) {
  if (filename === void 0) filename = 'sfc.vue'

  var originalCodeString = resource.toString()
  var parsedSFC = compiler.parseComponent(originalCodeString, { pad: 'line' })
  ...
  // parse template
  var template = parsedSFC.template

  //Consider the start of template for debugging
  //

  var templateStartIndex = parsedSFC.template.start
  var templateStartLineNumber = originalCodeString
    .substring(0, templateStartIndex)
    .split(newLine).length

  // Get tags and location of tags from template
  //
  var templateASTNodes = []
  var templateFragments = parse5.parseFragment(parsedSFC.template.content, {
  ...
  var generatedTemplateCode = DEFAULT_OUTPUT.template
  if (template) {
  ...
  // parse script
  var script = parsedSFC.script

  var generatedScriptCode = DEFAULT_OUTPUT.script
  if (script) {
    var scriptContentStartLine = getFirstValidLine(script.content)
  ...
  if (mappings) {
    ...
    var exportDefaultLineNumber = originalCodeString
      ...
      .split(newLine).length
    var firstLineOfScript = getFirstValidLine(script.content) + 1
    var scriptEndLine = script.content.split(newLine).length
    var endLines = output.split(newLine).length - 1

    var generatedCodeCursor = endLines

    // Mapping from Bottom to Up from the endlines of script and generated code to first valid line of parsed script content
    for (
      var scriptCodeCursor = scriptEndLine;
  ...
  // add render funtion
  var beautifiedRender = jsBeautify.js_beautify(
    addvm(generatedTemplateCode.render),
  )
  ...
  //
  var renderEndLine = beautifiedRender.split(newLine).length - 1
...
function getFirstLineWithText(content, text) {
  var contentByLine = content.split(newLine)
  if (!content.includes(text)) {
    return contentByLine.length
  }
  return contentByLine.findIndex(function(line) {
...
function getFirstValidLine(content) {
  var firstImport = getFirstLineWithText(content, 'import')
  var firstExport = getFirstLineWithText(content, 'export')
  var firstNonCommentedLine = getFirstLineWithoutText(content, '//')
...
function sourceMapAstInPlace(sourceMap$1, babelAst) {
